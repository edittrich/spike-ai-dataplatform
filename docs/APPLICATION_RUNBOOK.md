...
5. **[`scripts/ground_fibo_ontology_uris.py`](../scripts/ground_fibo_ontology_uris.py):** Grounds table entities to W3C FIBO class URIs (e.g. `Party` $\rightarrow$ `https://www.omg.org/spec/Commons/PartiesAndSituations/Party` — genuinely defined in a separate OMG "Commons" ontology FIBO itself imports, not under FIBO's own `spec.edmcouncil.org/fibo/ontology/` namespace; see D5 in the hardening plan). 7 of the 19 `FIBO_GROUNDING_MAP` entries have been individually re-verified against the real upstream FIBO source; the remaining 12 are best-effort, not independently confirmed — see the map's own module comment.
