...
3. **Semantic & governance** — Cube.js cubes in `cube/model/cubes/*.yml` (`:4000`), including `ref_country`/`ref_currency`/`ref_nace_industry` (explicit schema-qualified `sql_table:`, since `CUBEJS_DB_SCHEMA=financial` only affects introspection) joined into the party/deposit/loan cubes for AML/country/industry slicing, plus 4 `pre_aggregations` rollups served by the dev-mode image's bundled `cubestore-dev` (no separate Cube Store service needed — see `docker-compose.yml`'s `cube` service comments); OpenMetadata catalog (`:8585`) with domains/contracts under `contracts/*.yaml`; W3C FIBO ontology grounding (`ontology/*.ttl`).
