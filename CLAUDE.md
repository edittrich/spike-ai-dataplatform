...
The MCP SSE endpoint (`mcp_sidecar`, `:8001`) and the Cube.js semantic layer (`:4000`) both enforce real authentication now — `MCP_API_KEY` (bearer token, checked by `BearerAuthMiddleware` in `mcp_server/financial_data_mcp_server.py`) and two bearer tokens checked by `cube/cube.js`'s `checkAuth` via `crypto.timingSafeEqual` (H1 in the hardening plan): `CUBEJS_API_SECRET` (privileged role — full semantic-layer access) and `CUBEJS_API_SECRET_RESTRICTED` (restricted role — general BIAN/FIBO access; `queryRewrite`'s `AML_RESTRICTED_MEMBERS` blocks AML risk classification fields for this role specifically, a real row-level/tenant authorization boundary, not just authentication). Neither MCP_API_KEY nor either Cube.js secret has a hardcoded fallback; in SSE mode an unset `MCP_API_KEY` makes the server refuse to start at all (fail closed, not open), and an unset `CUBEJS_API_SECRET`/`CUBEJS_API_SECRET_RESTRICTED` makes Cube.js reject every request. `cube` now also runs in production mode (`CUBEJS_DEV_MODE=false`) against a standalone single-instance `cubestore` service (`cubejs/cubestore:v1.7.17`, also acting as its own refresh worker via `CUBEJS_REFRESH_WORKER=true` so `pre_aggregations` rollups actually build) — the Developer Playground is gone as a result. The MCP server, `scripts/hybrid_rag_retriever.py`, and Cube.js itself all connect to Postgres as dedicated non-superuser roles (`mcp_readonly`/`cube_readonly`, deliberate siblings not a shared login — `MCP_PG_READONLY_USER`/`PASSWORD` and `CUBE_PG_READONLY_USER`/`PASSWORD`) rather than the `postgres` superuser used elsewhere in the pipeline — see `supabase/migrations/20260807151500_create_mcp_readonly_role.sql` and `20260808150000_create_cube_readonly_role.sql`; configure both roles' passwords with `python3 scripts/configure_readonly_role.py`.
...
Beware also: `OPENMETADATA_JWT_TOKEN` (the `ingestion-bot`'s credential — H10 in the hardening plan) is a genuinely time-bounded 90-day token, not a permanent one; letting it expire silently breaks catalog writes and the MCP `search_data_catalog` tool. `python3 scripts/rotate_openmetadata_bot_token.py` checks/renews it (idempotent, no-ops unless expiring within 14 days; needs `OPENMETADATA_ADMIN_EMAIL`/`OPENMETADATA_ADMIN_PASSWORD` in `.env` to actually mint a new one) and is already wired into `scripts/bootstrap_platform.sh` and, independent of that, into a daily Dagster schedule (`orchestration/definitions.py`'s `bot_token_rotation_daily`) — normal operation shouldn't need this run by hand. Note also that a freshly-minted bot token can take up to roughly a minute before OpenMetadata's own internal auth cache recognizes it on every API path (observed live: an immediate call right after rotation can 401 once, then succeed on retry a few seconds later) — expected propagation delay, not a rotation failure.
